...
namespace MathParserTK
{
    ...
    public class MathParser
    {
        ...
        //add Modulo
        private const string Modulo = OperatorMarker + "%";
...
namespace StringExtensions
{
    ...
    {
        static MathParser mp = new MathParser('.');
        /////////////////
        public static string ToMath(this string s)
        {
            try
            {
                if (s.IsArismetic())
                {
                    if (s.IsCompareable())
                    {
                        var ss = CompareSplit(s);
                        var m1 = mp.Parse(ss[0]);
                        var m2 = mp.Parse(ss[2]);
                        var ret = Compare(m1, ss[1], m2);
                        return TrueOrFalseToDouble(ret).ToString();
                    }
                    else
                    {
                        return mp.Parse(s).ToString();
                    }
                }
                else
                {
                    ...
                    {
                        var ss = CompareSplit(s);
                        var ret = Compare(ss[0], ss[1], ss[2]);
                        return TrueOrFalseToDouble(ret).ToString();
                    }else{
                        return s;
                    }
        ...
        public static bool TryMath(this string s,out string result)
        {
            try
            {
                if (s.IsArismetic())
                {
                    if (s.IsCompareable())
                    {
                        ...
                        var m2 = mp.Parse(ss[2]);
                        var ret = Compare(m1, ss[1], m2);
                        result = TrueOrFalseToDouble(ret).ToString();
                        return true;
                    }
                    else
                    {
                        result= mp.Parse(s).ToString();
                        return true;
                    }
                }
                else
        ...
        public static string ToValue(this string s, ref Dictionary<string, string> dic)
        {
            var buf = new StringBuilder(s);
            var ddic=dic.OrderByDescending(dd => dd.Key.Length);
            foreach (var dd in ddic)
                buf = buf.Replace(dd.Key, dd.Value);
            return buf.ToString();
        }

        public static string ToValue(this string s, Dictionary<string, string> dic)
        {
            var buf = new StringBuilder(s);
            var ddic = dic.OrderByDescending(dd => dd.Key.Length);
            foreach (var dd in ddic)
        ...
        static bool Compare(string d1, string ch, string d2)
        {
            if (ch == "==") return d1 == d2;
            else if (ch == "!=") return d1 != d2;
            throw new ArgumentException($"channel miss match => ch={ch}");
        }
        static bool Compare(double d1, string ch, double d2)
        {
            if (ch == "==") return d1 == d2;
            else if (ch == "!=") return d1 != d2;
            else if (ch == "<=") return d1 <= d2;
        ...
        ////////////////
        ////////////////
        public static bool IsArismetic(this string s)
        {
            var buf = s;
            var re = @"^[0-9\.\|\=\<\>\&\(\)\!\%\+\-\*\/\^ ]+$";
            return Regex.IsMatch(buf, re);
        }
        static double TrueOrFalseToDouble(bool x)
        {
            if (x) return 1;
            else return 0;
        }
        public static bool IsCompareable(this string s)
        {
            var buf = s;//IsValuable(s) ? Valuable(s) : s;
            var re = @"^([^=<>!]+)(==|<=|>=|!=|<|>)([^=<>!]+)+$";
            return Regex.IsMatch(buf, re);
        }
        public static string[] CompareSplit(string s)
        {
            var buf = s;//IsValuable(s) ? Valuable(s) : s;
            var re = @"^([^=<>!]+)(==|<=|>=|!=|<|>)([^=<>!]+)+$";
            return Regex.Match(buf, re).Groups.Values
             .Select(dd => dd.Value.Trim())
             .Skip(1).ToArray(); //str1, ch , str2
        }
    ...
    public static class StringExtensions2
    {
        ...
        {
            float ret = 0.0f;
            bool a = float.TryParse(me, out ret);
            return ret;
        }
        public static int ToInt(this String me)
        {
            int ret = 0;
            bool a = int.TryParse(me, out ret);
            return ret;
        }
    }
